...
public class PieChart extends JComponent {
    ...
    private int labelArc = 10;

    private float pieSize = 0.5f;

    private Color labelForeground = new Color(34, 34, 34, 255);
    private Color labelBackground = new Color(255, 255, 255, 64);
    private Color labelBorderColor = new Color(255, 255, 255, 127);

    private Color pieForeground = new Color(34, 34, 34, 255);
    private Color pieBorderColor = new Color(0, 0, 0, 0);
    
    private boolean pieLabeled = true;
    private boolean popupLabelFlow = false;
    
    private float hoverAlpha = 0.6f;

    private LabelType labelType = LabelType.Center;
    ...
    private void drawPopupLabel(Graphics2D g2, double size, double angle, double labelX, double labelY, String text, String detail) {
        if (labelType == LabelType.Popup) {
            ...
            if (!isPopupLabelFlow()) {
                ...
                RoundRectangle2D rec = new RoundRectangle2D.Double(recX, recY, width, height, labelArc, labelArc);
                g2.fill(rec);
        ...
        } else if (labelType == LabelType.Center) {
            float fontSize = (float) (getFont().getSize() * size * 0.0035f);
            double spaceV = size * 0.01f;
            double spaceH = size * 0.01f;
            ...
            double width = Math.max(r1.getWidth() + spaceH * 2, r2.getWidth() + spaceH * 2);
            double height = r1.getHeight() + r2.getHeight() + spaceV * 2;
            
            double XCenter = (getWidth() / 2) - (width / 2) + spaceH * 2;
            double YCenter = (getHeight() / 2) - (height / 2);

            g2.setColor(labelForeground);

            g2.setFont(getFont().deriveFont(Font.PLAIN, fontSize));
            g2.drawString(text, (float) XCenter, (float) (YCenter + fm1.getAscent() + spaceV));
            g2.setFont(getFont().deriveFont(Font.BOLD, fontSize));
            g2.drawString(detail, (float) XCenter, (float) (YCenter + height - r2.getHeight() + fm2.getAscent() - spaceV));
        }
    ...
    public void setLabelBorderColor(Color c) {
        this.labelBorderColor = c;
        repaint();
    }

    public void setPieForeground(Color c) {
        this.pieForeground = c;
    }

    public void setPieBorderColor(Color c) {
        this.pieBorderColor = c;
    }

    public boolean isPieLabeled() {
        return pieLabeled;
    }

    public void setPieLabeled(boolean pieLabeled) {
        this.pieLabeled = pieLabeled;
        repaint();
    }

    public boolean isPopupLabelFlow() {
        return popupLabelFlow;
    }

    public void setPopupLabelFlow(boolean popupLabelFlow) {
        this.popupLabelFlow = popupLabelFlow;
        repaint();
    }
